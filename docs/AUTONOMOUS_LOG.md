...
## 2026-09-01 (autonomous run, POLISH)

**Mode:** POLISH
**Action:** Polished the `nav2-config` pair (#54), closing the urdf → tf → nav2 chain; found and removed a live pointer at an **end-of-life ROS 2 distro** and gave the reviewer the vocabulary to catch the class.
**Files touched:** `skills/nav2-config-builder.skill`, `skills/nav2-config-checklist-reviewer.skill`, `docs/skill-polish-log/nav2-config-builder.md`, `examples/nav2-config-builder/README.md`, `examples/nav2-config-checklist-reviewer/README.md`, `STATUS.md`, `docs/AUTONOMOUS_LOG.md`
**Tests:** N/A (no test suite in this repo yet)
**Skill count:** 38 builders / 38 reviewers / 100% paired
**Open issues:** 20
**Notes:** The headline finding is a category this repo had not hit before. Every previous edition defect was a builder that named **no** version; this one named the **wrong** one — `per ROS 2 Humble or Iron Nav2`, and **Iron Irwini has been end-of-life since November 2024**, twenty-two months. Its reviewer pinned nothing at all (`per ROS 2 Nav2 best practices`), so the pair would have accepted a configuration built against an unsupported distro without a murmur. Worth recording precisely because **ros2 is not in the task's safety-critical set, so the mandatory edition-verification step did not fire** — I ran it anyway on the grounds that the builder carried a version claim, which is the rule W35 wrote down, and it is what caught this. A wrong baseline is strictly worse than an absent one: absence prompts a question, a wrong pin answers it. Second judgement call: I declined to assert a parameter name. The Nav2 Lyrical announcement (2026-08-25, seven days old) describes "an symmetric inflation layer to incentivize left- or right-hand side passing" — internally inconsistent, since a left/right bias needs an *a*symmetric inflation — and the Kilted→Lyrical migration page was not retrievable (the fetch tool is restricted to URLs surfaced by search, and search returned only the Kilted source page). Rather than record a plausible-sounding name, the builder carries the feature with an explicit instruction to verify it against the migration guide first. That is the W35 `iso3691-4` lesson running the other way: last time the *task file* was stale and the builder right; this time the upstream *announcement* is ambiguous and the correct move is to write the uncertainty into the artifact instead of resolving it by guess. Everything else was verified and is solid: Lyrical Luth as LTS to May 2031, the `enable_stamped_cmd_vel` / `TwistStamped` default flip at Kilted, `nav2_ros_common` and `nav2::LifecycleNode`, and `inscribed_obstacle_cost_value` default 99. The `cmd_vel` flip is the best single check the reviewer gained — it produces a robot that plans normally and never moves, with no error anywhere. Re-ran `scripts/audit_pair_editions.py` after the edit: 1 mismatch / 10 asymmetries, unchanged, so no new drift was introduced. The three-week `urdf → tf → nav2` sequence is now closed and it paid: the frame contract was transcribed once, in one direction that was previously documented only from the other end.
**Follow-ups:**
- **Verify the Lyrical directional/asymmetric inflation parameter name** against `docs.nav2.org` Kilted→Lyrical migration when that page is reachable, then tighten the hedge in `nav2-config-builder`. Carried deliberately; do not resolve it by guess.
- W36 remaining: #55 `iso9283-performance-test` (Wed, thinnest builder in the repo at 6 body lines), #56 `operator-training-matrix` (Thu, **safety-critical domain — mandatory edition check fires**).
- **New standing rule earned this run:** run the edition check on any builder carrying a version claim, regardless of domain. The safety-critical list is a floor, not the trigger. This run is the evidence.
- `ros2-system-architecture` (9 body lines) is now the last inverted ros2 pair and inherits publisher assignments from `tf-tree-spec`; `robot-field-acceptance` (9 lines) is the other survivor.
- Reciprocal-edit loops still open: `eoat-spec`→safety-io/iso13849, `robot-hil-test-catalog`. The `tf-tree-spec`→nav2 loop **closed this run**.
- `scripts/audit_descriptions.py` still uncommitted — fourth consecutive run noting it.
...
## 2026-09-03 (autonomous run, POLISH)

**Mode:** POLISH
**Action:** Polished the `operator-training-matrix` pair (#56) — the last operational builder at import baseline (7 body lines) now keeps operator / programmer / maintenance / integrator duties distinct, carries OSHA 1910.147(c)(7) authorized / affected / other status by name as an attribute separate from role, splits training delivered from competency demonstrated with an evidence type per row, lists retraining as (c)(7)(iii) conditions before interval, and pins ISO 10218-2:2025, ANSI/A3 R15.06-2025 (Parts 1–3) and ANSI/ASSP Z490.1-2024; reviewer rewritten as findings in lockstep. W36 closes 3/3 — fourth consecutive clean week.
**Files touched:** `skills/operator-training-matrix-builder.skill`, `skills/operator-training-matrix-checklist-reviewer.skill`, `docs/skill-polish-log/operator-training-matrix-builder.md`, `examples/operator-training-matrix-builder/README.md`, `examples/operator-training-matrix-checklist-reviewer/README.md`, `STATUS.md`, `docs/AUTONOMOUS_LOG.md`
**Tests:** N/A (no test suite in this repo yet)
**Skill count:** 38 builders / 38 reviewers / 100% paired
**Open issues:** 20
**Notes:** The mandatory edition check fired (operational) and found the *absent* class rather than the wrong class: both halves cited ISO 10218-2 and ANSI R15.06 with no year. All three anchors verified — ISO 10218-2:2025 (Feb 2025, Ed. 2), R15.06-2025 Parts 1/2 approved 2025-08-21 and Part 3 2025-10-07, and Z490.1 where the **2016 edition is superseded by 2024** (new to the suite; added as the evaluation/records reference). The judgement call worth recording is a near-miss on **ISO/TS 15066:2016**: a search summary stated it was withdrawn at stage 95.99. iso.org (cat. 62996) says Published, confirmed 2022, stage **90.92 to be revised**, successor ISO/AWI 15066-1 in development — the 95.99 line is the lifecycle template every ISO page renders, not the current stage. Had that been trusted, a safety-critical pair would carry "withdrawn" and this journal would have flagged the task file's `ISO/TS 15066:2016` row as a second stale entry. **It is not stale; the `ISO 3691-4:2020` row remains the only one.** The substantive defect was that the original builder inverted 1910.147's retraining logic — it had a "recertification interval" and none of the five (c)(7)(iii) conditions, while the regulation has the conditions and no interval. The reviewer now reserves top severity for the three silent failures (attendance as competency, LOTO status inferred from role, interval as sole trigger) and adds a chain check: the authorized roster must match the `loto-procedure` workbook's roster by name. Two process traps caught and written into the polish log: `zip -r` without `-D` inflates the entry count used as the preservation check, and a standards *table* cell `| ISO/TS 15066 | 2016 |` is invisible to `audit_pair_editions.py` (year must follow the number). Final audit: 1 mismatch / 10 asymmetries, unchanged.
**Follow-ups:**
- **W37 PLAN (Mon):** candidates in the order W36 set — (1) `iso10218-compliance-matrix` (boilerplate body on the compliance keystone, own body unpinned); (2) the 10 edition asymmetries as one bounded themed slot — `robot-cell-layout`, `robot-cell-scope`, `robot-sop`, `ssm-plan` and six others, pin the unpinned half, no body rewrites; (3) `ros2-system-architecture` (9) and `robot-field-acceptance` (9), the last two inverted pairs.
- **New check for the asymmetry slot:** grep the suite for `Z490.1-2016` before pinning anything; it is superseded by 2024 as of this run.
- Chain check added this run (`operator-training-matrix` roster ↔ `loto-procedure` roster) is one-directional: `loto-procedure-builder` (W31) does not yet say its roster must agree with the training matrix. Reciprocal-edit loop opened; add to the existing list (`eoat-spec`→safety-io/iso13849, `robot-hil-test-catalog`).
- Repack rule, now twice-earned: `zip -qr -X -D`. Consider putting a `scripts/repack_skill.sh` in the repo so it stops being retyped — same reasoning as `gen_status.py` and `audit_pair_editions.py`.
- Still carried: Lyrical directional-inflation parameter name (nav2, 09-01); `scripts/audit_descriptions.py` uncommitted (sixth run noting it); reviewer probe scripts as placeholders suite-wide (themed W37+ slot).
...
## 2026-09-15 (autonomous run, POLISH)

**Mode:** POLISH
**Action:** Polished the `ros2-system-architecture` pair (W37 carryover target #58) — builder rewritten from a 9-line tab glossary into a method, reviewer rewritten from eight scope bullets into nine rated check groups, safety boundary and implementation-status sections added to both halves.
**Files touched:** skills/ros2-system-architecture-builder.skill · skills/ros2-system-architecture-checklist-reviewer.skill · docs/skill-polish-log/ros2-system-architecture-builder.md · STATUS.md · docs/AUTONOMOUS_LOG.md
**Tests:** N/A (no test suite) — `gen_status.py` clean, `audit_pair_editions.py` unchanged (pair not listed either side), `unzip -t` clean on both repacked archives, distro table verified by web search.
**Skill count:** 38 builders / 38 reviewers / 100% paired
**Open issues:** 0
**Notes:** **Six-day gap in the schedule — this is the first run since 2026-09-08.** W37's Wednesday (#58), Thursday (#59), Friday DOCS, Saturday RELEASE and Sunday TRIAGE did not run, and neither did Monday's W38 PLAN, so there is no `WEEK-2026-W38.md` and today's target had to be selected without one. The selection rules pointed the same way the missing plan would have: #58 was the designated next target, is the second-thinnest builder in the suite, and sits directly above the three skills the preceding three weeks polished. Taking it was the obvious move; the unplanned week is the thing to flag, not the pick. **The ten-week request has been honoured — all 46 issues are closed and the open count is 0.** That resolves the rule-(a) deadlock, and from the next PLAN run rule (a) will select from a backlog that reflects real outstanding work rather than shipped work. It also means #59's 16-claim edition-asymmetry sweep is now closed as an issue but **not done as work** — `audit_pair_editions.py` still reports all 16 across the same 8 pairs. That is the single most important thing in this entry: the tracking is clean and the debt is not. Two judgement calls. First, the distro-baseline precedence: `nav2-config` says the baseline comes from `urdf-model-spec` and the URDF workbook wins on conflict, so this builder adopts the same rule rather than claiming precedence as the system-level document, which would have created two answers depending on which workbook was opened. Second, severity was recorded as high despite no factual error being found — the defect was absence, and a ROS 2 architecture document silent on QoS compatibility, executor reentrancy and the safety boundary is silent about its own worst failures. The safety boundary is new to this pair and is worth propagating: nothing checked whether a ROS 2 mechanism had been recorded as a risk reduction measure, and this suite contains the rated-hardware skills that should own that.
**Follow-ups:**
- **#59 is closed but unworked.** The 16 edition asymmetries across `cobot-hand-guiding` (3), `interlock-estop-architecture` (4), `robot-cell-layout` (3), `ssm-plan` (3), `iec62443-risk-assessment` (1), `iso3691-4-risk-assessment` (1), `robot-cell-scope` (1) and `robot-sop` (1) are all still present. Re-file it — next PLAN run should open it as the first W38/W39 target, with the same descope instruction as before.
- **No W38 plan exists.** Next Monday's PLAN covers two weeks of selection, and should run the reviewer-thinness ranking W37 proposed — this run confirmed the defect class is real, since the reviewer half here was thinner in substance than the builder.
- **Human action, third request:** the task file's reference table still says `ISO 3691-4:2020`; current edition is **2023**, which the builder already tracks correctly.
- **Human action, second request:** the task file's ISO/TS 15066:2016 entry should read "current edition, being superseded by ISO/AWI 15066-1", not "still current" (raised 2026-09-08).
...
## 2026-09-21 (autonomous run, PLAN)

**Mode:** PLAN
**Action:** Opened W39 on an empty board — stood up and committed the reviewer-thinness ranking as the successor selector, filed three weekly targets (#60-#62) plus two tracked-debt issues (#63-#64) that restore rule (a) from W40.
**Files touched:** `docs/weekly/WEEK-2026-W39.md` (new), `scripts/rank_reviewer_thinness.py` (new), `STATUS.md`, `docs/AUTONOMOUS_LOG.md`; GitHub issues #60-#64 created
**Tests:** N/A (no test suite in this repo yet) — verified by running `rank_reviewer_thinness.py`, `gen_status.py`, `audit_pair_editions.py` and `audit_reviewer_impl.py` against the tree, and by reading the three thinnest reviewer archives directly rather than trusting the line counts
**Skill count:** 38 builders / 38 reviewers / 100% paired by filename — 6/38 reviewers actually implemented
**Open issues:** 5 (was 0)
**Notes:** Three structural changes landed at once and the plan is mostly about them. The board was **empty** for the first time, so rule (a) returned nothing for an eighth week but for the opposite reason to the previous seven; **builder-thinness was exhausted** on 09-16; and Sunday's triage put the repo's largest defect (32 of 38 reviewers unimplemented) on record without it ever having been on the board. The response to all three was to file real debt as issues rather than invent another measure — **#63 and #64 are deliberately not scheduled**, and from W40 rule (a) has genuine outstanding work to return for the first time since W31. On the successor selector: I committed it as `scripts/rank_reviewer_thinness.py` for the reason `gen_status.py` gives, and the docstring records the trap, because the obvious formulation is wrong and I nearly shipped it. Sorting by `reviewer - builder` delta ranks `dataset-documentation` first at -42, but only because its *builder* was grown to 109 lines last month; its 67-line reviewer is one of the healthiest in the repo. **Absolute reviewer size is the signal, not the delta** — it put `robot-cell-layout` (6 body lines, zero checks) first where delta had it fourth, and reading the three thinnest archives confirmed it: `safety-io-matrix` at 13 lines names real checks and dates its standards and is *not* a defect, so line count alone would have burned a slot on a healthy pair. Two judgement calls. First, **two of three targets are cell-design**, which breaks the spread rule; taken deliberately because the ranking's top two both land there (that domain was imported as a block and its reviewers were never written past a topic list) and because taking them consecutively closes 7 of the 9 open edition asymmetries as a side effect of real polish, which is strictly better than the mechanical sweep #59 tried and failed to be. Second, **I did not backfill a W38 plan** — the week was already worked on the fallback selector, and a retrospective plan would be fiction; W39 absorbs the two selection decisions W38 should have made instead. One thing the human should read: the plan's own runway note concedes that **reviewer-thinness is spent after this week too** — only 3 reviewers sit under 15 lines and two are taken now. Both thinness measures were detecting the same underlying thing, unpolished imports, and the durable successor is #63's tier ranking, which sorts by whether the code exists at all.
**Follow-ups:**
- **Human, fourth request and now the top process risk:** verify the 7:30 schedule actually fires. Missed slots are W35 Fri-Sun, W37 Thu-Sun, W38 Mon, 09-13 and 09-14 — three missed weekends and the first missed weekday in four weeks. Content debt is recoverable; a loop that does not run is not.
- **Human, #63 needs a decision:** authorize an implementation week, mark the tier-B/C contracts honestly, or add a reviewer-implementation column to STATUS.md. The third is cheap and worth doing regardless — "38/38 paired (100%)" currently reads as 100% working.
- **W40 PLAN should adopt tier (C -> B -> A) as the primary selector.** Both thinness measures are spent.
...
**Open issues:** 5 (#61 worked today and left open; POLISH mode does not authorize issue writes, so no comment was posted and closing is for the human)
